...
26 . &nbsp; Ter  --  24/06  --  <span style="color:standard"> Atividade Avaliativa (opicional) valendo 30% da nota da 2ª VA -- </span> \
27 . &nbsp; Seg  --  30/06  --  <span style="color:standard"> Aula de Exercícios </span> \
<span style="color:red"> 28 . &nbsp; Ter  --  01/07  --  2ª VA </span> -- [gabarito](../provas/2025-1-apva2-turmaM1-c3-gabarito.pdf) \
---

**Verificações Finais**

<span style="color:red"> 29 . &nbsp; Seg  --  07/07  --  2ª Chamada </span> -- [gabarito](../provas/2025-1-vr-turmaM2-c3-gabarito.pdf) \
30 . &nbsp; Ter  --  08/07  --  <span style="color:standard"> Atendimento no gabinete </span> \
<span style="color:red"> 31 . &nbsp; Seg  --  14/07  --  VS </span> Não houveram alunos de VS :raised_hands:\
...
- Vetores Normal e Binormal e triedro de Frenet
- Componentes normal e tangencial da aceleração
- Teorema da Função Implícita e derivação implícita.
- Diferenciabilidade e Diferenciais.
...
dados = pd.read_csv('2025-1-calculoIII-A-notas.csv')
insc = len(dados['CPF']) 

print('## 1ª Verificação {-}')

p1 = dados['VA1'].replace('ausente', np.nan).str.replace(',', '.').astype(float).dropna().to_numpy()
N1=len(p1)
...
max_freq = int(max([patch.get_height() for patch in plt.gca().patches]))  
...
p2 = dados['N2'].replace('ausente', np.nan).str.replace(',', '.').astype(float).dropna().to_numpy()
...
# Medidas de Dispersão
variancia = np.var(p2, ddof=2)  # ddof=1 para amostra (não população)
desvio_padrao = np.std(p2, ddof=2)
amplitude = np.max(p2) - np.min(p2)
q1, q3 = np.percentile(p1, [25, 75])
iqr = q3 - q1
